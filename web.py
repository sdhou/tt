...
tokenizer = AutoTokenizer.from_pretrained("oye", trust_remote_code=True)
model = AutoModel.from_pretrained("oye", trust_remote_code=True)
model = load_lora_config(model)
model=model.half().cuda()
model = model.eval()
model.load_state_dict(torch.load(f"output1\\chatglm-6b-lora.pt"), strict=False)

a='好的，我明白了。请问有什么关于青浦幸福云的问题我可以帮您解答吗？'
...
2、数据中心功能，给予所有工作人员，根据标签快速筛选到指定居民，并对于这类居民用户进行后续服务。""",a),]
...
demo.queue().launch(server_name='0.0.0.0',server_port=port,share=False, inbrowser=True)
